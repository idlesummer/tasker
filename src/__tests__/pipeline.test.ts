...
describe('pipe', () => {
  ...
  describe('basic functionality', () => {
    it('should execute a single task successfully', async () => {
      ...
      const pipeline = pipe<TestContext>([task])
    ...
    it('should execute multiple tasks sequentially', async () => {
      ...
      const pipeline = pipe<TestContext>(tasks)
      const result = await pipeline.run({})
    ...
    it('should handle empty pipeline', async () => {
      const pipeline = pipe<TestContext>([])
  ...
  describe('context management', () => {
    it('should merge context updates between tasks', async () => {
      const tasks: Task<TestContext>[] = [
        {
          name: 'Task 1',
          run: async () => ({ message: 'Hello', count: 1 }),
        },
        {
          name: 'Task 2',
          run: async () => ({ data: ['item1', 'item2'] }),
        },
      ]
    ...
    it('should handle tasks that return void', async () => {
      const task: Task<TestContext> = {
        ...
        run: async () => {
          // Task that doesn't return anything (side effects only)
        },
      }
  ...
  describe('success messages', () => {
    ...
    it('should pass duration to onSuccess callback', async () => {
      const onSuccess = vi.fn((ctx, duration) => `Done in ${duration}ms`)
